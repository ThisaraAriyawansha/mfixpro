...
function emailFooter(brandName: string, year: number) {
  return `
    <div style="padding:16px 24px;border-top:1px solid #eee;font-family:Arial,Helvetica,sans-serif">
      <p style="color:#aaa;font-size:11px;margin:0 0 4px">© ${year} ${escapeHtml(brandName)} - automated message, please don't reply.</p>
      <p style="color:#ccc;font-size:11px;margin:0">Design &amp; Developed by plexCode</p>
    </div>
  `;
}

function emailShell(body: string, brandName: string, year: number) {
  return `
    <div style="background:#f4f4f5;padding:32px 16px;font-family:'Segoe UI',Helvetica,Arial,sans-serif">
      <div style="max-width:420px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden">
        <div style="background:#0a0a0a;border-bottom:4px solid #e30613;padding:20px 24px">
        ...
        ${emailFooter(brandName, year)}
      </div>
    </div>
  `;
}

export function changeEmailTemplate(newEmail: string, link: string, shopName: string, currentEmail?: string) {
  ...
  return emailShell(
    `
      ...
      <a href="${link}" style="background:#e30613;color:#fff;font-size:13px;padding:10px 22px;text-decoration:none;border-radius:6px;display:inline-block">
        Confirm Email
      </a>
...
// Job update emails are sent on behalf of the shop running M-Fixpro, not
// the M-Fixpro product itself, so they get their own shell branded with
// the shop's own name/contact details (from ShopSettings) instead of "M-Fixpro".
...
export function jobUpdateTemplate(params: {
  ...
  return shopEmailShell(
    `
      ...
      ${shop.phone ? `
      <a href="tel:${escapeHtml(shop.phone)}" style="background:#e30613;color:#fff;font-size:13px;padding:10px 22px;text-decoration:none;border-radius:6px;display:inline-block;margin-top:8px">
...
export function passwordResetOtpTemplate(otp: string, shopName: string) {
  const year = new Date().getFullYear();
  return emailShell(
    `
      <h1 style="font-size:16px;margin:0 0 12px">Reset your password</h1>
      <p style="margin:0 0 20px;font-size:14px;line-height:1.5">
        Use the code below to reset your ${escapeHtml(shopName)} password. It expires in 10 minutes.
      </p>
